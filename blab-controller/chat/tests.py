...
class ConversationTest(TestCase):
    ...
    def test_message_from_non_existing_sender(self) -> None:
        ...
        m1 = Message.objects.create(type=Message.MessageType.TEXT,
                                    text='Hi',
                                    conversation=self.c,
                                    sender=self.p1)
        with self.assertRaises(ValidationError):
            Message.objects.create(type=Message.MessageType.TEXT,
                                   text='Hello',
                                   conversation=self.c,
                                   sender=self.p3)
    ...
    def test_message_from_system(self) -> None:
        ...
        m2 = Message.objects.create(type=Message.MessageType.TEXT,
                                    text='System message',
                                    conversation=self.c,
                                    sender=self.psys)
